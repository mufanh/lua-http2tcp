...
function _M.set_cookie(cookie_record)
    if type(cookie_record) ~= "table" then
        return
    end
    -- compatible with Old Internet Explorer

    if cookie_record.max_age then
        cookie_record.expires = ngx.cookie_time(ngx.time() + cookie_record.max_age)
    end
    local cookie, err = ck:new()
    if not cookie then
        log(ngx.WARN, "set cookie fail: ", err)
        return
    end
    local ok, err = cookie:set(cookie_record)
    if not ok then
        log(ngx.WARN, "set cookie fail: ", err)
    end
end
